...
class UsbBootLoader():
	...
	def EraseUserFlash(self):
		...
		try:  
			for address in range( self.bootloader_offset, self.program_flash_size, self.erase_frame_size ) :
	...
	def WriteFlash(self, start_address, end_address, write_eeprom, write_config = False, write_id = False):
		...
		max_num_frames = self.max_program_flash_size / self.erase_frame_size
		...
		for i in range(0,num_frames):
			...
			if ( valid_frame ) :
				 ...
				 if ( j < ( self.bootloader_offset / self.erase_frame_size ) ) :
					  ...
					  print "HEX file out of bounds!!"
						...
						# usb_driver.EraseFlashBlock( 1, addr_u, addr_h, addr_l )
						#print "self.usb_driver.EraseFlashBlock( 1, " + str( addr_u ) + " " + str( addr_h ) + " " + str( addr_l ) + " )"
		
						block_offset = 0
		
						for i in range( 0, self.erase_frame_size / self.program_flash_frame_size ) :

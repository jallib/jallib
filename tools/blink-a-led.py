...
""" Create and compile blink-a-led samples.        
  ...
  Description: Python script to create blink-a-led samples       
               ...
               - Vvalidate the sample program                              
               - Compile the sample program                               
               - Ccheck the compiler output for errors and warnings        
               if all OK:                                               
  ...
  Notes:                                                                  
   ...
   - when a datasheet is renumbered (e.g. 41234 -> 40001234),             
     the ircf table must be modified accordingly.     
...
# --- input   
base   = os.path.join("/", "media", "nas")               # base of all
home   = os.path.join("/", "home", "robh")               # home directory
jallib = os.path.join(base, "jallib")                    # jallib base
devprod = os.path.join(jallib, "include", "device")      # production device files
devtest = os.path.join(home, "jal", "pic2jal", "test")   # test device files
incljal = os.path.join(jallib, "include",  "jal")        # jal library        

compiler = os.path.join(jallib, "compiler", "jalv2")     # compiler executable
...
# OSCCON_IRCF settings for 4 MHz INOTSC per datasheet number
ircf =   { 30221    : "--",
           30292    : "--",
           30325    : "--",
           30430    : "--",
...
# Table with the names of PICs for which an extra INTOSC blink sample            
...
def validate_jalfile(jalfile):
   ...
   """
   cmdlist = ["python", os.path.join(jallib, "tools", "jallib.py"), "validate", jalfile]
   try:
      log = subprocess.check_output(cmdlist, stderr=subprocess.STDOUT, shell=False)
   except subprocess.CalledProcessError as e:         # 
      flog = os.path.split(jalfile)[1][:-4] + ".vlog" 
      fp = open(flog, "w")
      print >>fp, e.output                            # save compiler output
      fp.close()  
...
def compile_sample(runtype, pgmname):
   ...
   flog = pgmname[:-3] + "log"                     # compiler output
   ...
   try:
      ...
      numerrors = loglist[-4]                      # get number of errors
      numwarnings = loglist[-2]                    # and warnings
      if ( (numerrors == "0") and (numwarnings == "0") ):
#        print "Compilation of", pgmname, "successful!"  # OK! (zero errors, zero warnings)
         subprocess.call(["mv", "--force", pgmname, os.path.join(smpdir, pgmname)], shell=False)  # move sample
         return True
      else:
         print "Compilation of", pgmname, "gave", numerrors, "errors", numwarnings, "warnings"
         fp = open(flog, "w")
         print >>fp, log                           # save compiler output
         fp.close() 
         return False 
   except subprocess.CalledProcessError as e:         
      fp = open(flog, "w")
      print >>fp, e.output                         # save compiler output
      fp.close()  
      print "Compilation error(s) with sample", pgmname, "see", flog
      return False 
...
def build_sample(pic, pin, osctype, oscword):
   ...
   if (osctype.startswith("INTOSC")  &            # no 4 MHz INTOSC for some PICs 
       (var["datasheet"] in (39663 , 39682, 39760, 39762))):
   ...
   fp.write("--\n")
   fp.write("-- Author: " + ScriptAuthor + ", Copyright (c) 2008..2015, all rights reserved.\n")
   fp.write("--\n")
   fp.write("-- Adapted-by:\n")
   fp.write("--\n")
   ...
   if (osctype == "HS"):                                # HS crystal or resonator 
      ...
      fusedef_insert("fosc2", "ON", "oscillator: OSC")
   ...
   if (osctype == "HS"):                                   # HS crystal / resonator 
      ...
      if ("ircfwidth" in var):                           
         if (var["ircfwidth"] > 0): 
            ds = var["datasheet"]
            if (ds not in ircf):
               print "Missing OSCCON_IRCF specification for", ds
            elif (len(ircf[ds]) != var["ircfwidth"]):
               print "Conflict between width of OSCCON_IRCF (", var["ircfwidth"], ") and ", len(ircf[ds])
            elif (ircf[ds] == "--"):
   ...
   fp.write("--\n")
   fp.write("include delay                       -- library with delay procedures\n")
   fp.write("--\n")
   ...
   fp.write("   led = ON\n")
   fp.write("   delay_100ms(1)\n")
   fp.write("   led = OFF\n")
   fp.write("   delay_100ms(4)\n")
   fp.write("end loop\n")
   fp.write("--\n")
...
def main(runtype, devs):
   """ Create one or more blink-a-led samples for every device file
       Arguments: runtype: PROD or TEST
                  devs: list of device files for which to build blink sample
   """
   sample_count = 0
   for dev in devs: 
      if (runtype == "PROD"):                               # production device file   
         ...
         jalfile = os.path.join(devtest, dev)
...
if (__name__ == "__main__"):
   """ Process commandline arguments, start process, time execution
   """
   if (len(sys.argv) > 1):
